...
export function SparqlExplorer() {
  ...
  async function runQuery() {
    setIsRunning(true);
    const nextRows = await runSparqlQuery(query);
    setRows(nextRows);
    setIsRunning(false);
  }

  const selectedTemplate = sparqlTemplates.find((item) => item.id === templateId);

  return (
    <section id="sparql" className="border-b border-slate-200 bg-slate-50 px-6 py-20">
